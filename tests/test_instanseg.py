...
def test_inference():
    ...
    example_image_folder = Path(
        os.path.join(os.path.dirname(__file__), "../instanseg/examples/")
    )
    ...
    image_array, pixel_size = instanseg_brightfield.read_image(
        example_image_folder / "HE_example.tif"
    )
    ...
    instanseg_fluorescence = InstanSeg(
        "fluorescence_nuclei_and_cells", verbosity=0, device=device
    )
    image_array, pixel_size = instanseg_fluorescence.read_image(
        example_image_folder / "Fluorescence_example.tif"
    )
    ...
    instanseg_fluorescence.eval_small_image(
        torch.randn(1, 1, 256, 256), 1, rescale_output=True
    )

    instanseg_fluorescence.eval_small_image(
        torch.randn(1, 1, 256, 256), 1, rescale_output=False
    )
    ...
    label, img = instanseg_fluorescence.eval_medium_image(
        input, 0.5, rescale_output=True
    )
    ...
    label, img = instanseg_fluorescence.eval_medium_image(
        input, 1, rescale_output=True, normalise=True
    )
    assert label.shape[-2:] == input.shape[-2:]

    input = torch.randn(1, 1, 2560, 2560)
    label, img = instanseg_fluorescence.eval_medium_image(
        input, 0.1, rescale_output=True
    )
    assert label.shape[-2:] == input.shape[-2:]
