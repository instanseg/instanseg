...
parser.add_argument(
    "-db", "--debug", default=False, type=lambda x: (str(x).lower() == "true")
)
parser.add_argument(
    "-save_ims", "--save_ims", default=False, type=lambda x: (str(x).lower() == "true")
)
parser.add_argument(
    "-data",
    "--dataset",
...
parser.add_argument(
    ...
    help="Optimize postprocessing parameters",
)
parser.add_argument(
    "-tta",
    "--tta",
...
parser.add_argument(
    ...
    default="default",
    type=str,
    help="Either 'default' or 'best_params'",
)
...
parser.add_argument(
    ...
    default="Validation",
    type=str,
    help="Validation or Test or Train",
)
parser.add_argument(
...
def instanseg_inference(
    ...
    with torch.no_grad():
        ...
        if params is not None:
            with torch.amp.autocast("cuda"):
                lab = postprocessing_fn(
                    pred, **params, window_size=parser_args.window_size
                )
        else:
            with torch.amp.autocast("cuda"):
                lab = postprocessing_fn(
                    pred, img=imgs, window_size=parser_args.window_size
                )
        lab = lab.cpu().numpy()

        if tta:
            lab = method.TTA_postprocessing(
                imgs[
                    None,
                ],
                model,
    ...
    print(
        ...
        time_dict["preprocessing"],
        "Time spent in model:",
        time_dict["model"],
        "Time spent in postprocessing:",
        time_dict["postprocessing"],
    )
...
if __name__ == "__main__":
    ...
    model, model_dict = load_model(
        path=parser_args.model_path, folder=parser_args.model_folder
    )
    ...
    if parser_args.loss_function.lower() == "instanseg_loss":
        ...
        raise NotImplementedError(
            "Loss function not recognized", parser_args.loss_function
        )
    ...
    if "inference_folder" not in parser_args or parser_args.inference_folder is None:
        ...
        val_images, val_labels, val_meta = _read_images_from_pth(
            args=parser_args, sets=[parser_args.test_set], dataset=parser_args.dataset
        )
    ...
    val_data = [
        Augmenter.duplicate_grayscale_channels(
            *Augmenter.to_tensor(img, label, normalize=True)
        )
    ...
    if parser_args.pixel_size is not None and parser_args.pixel_size != "None":
        print(
            "Warning, rescaling image and ground truth labels to pixel size:",
            parser_args.pixel_size,
            "microns/pixel",
        )
        ...
        val_data = [
            Augmenter.torch_rescale(
                img,
                label,
                current_pixel_size=val_meta[i]["pixel_size"],
                requested_pixel_size=parser_args.pixel_size,
                modality=val_meta[i]["image_modality"],
                crop=False,
            )
            for i, (img, label) in enumerate(val_data)
        ]
    ...
    area = np.array(
        [(len(label[label > 0].flatten())) / f for f, label in zip(freq, val_labels)]
    )  # this will break for images with 0 labels
    ...
    if parser_args.optimize_hyperparameters:
        ...
        pd.DataFrame.from_dict(params, orient="index").to_csv(
            output_path / "best_params.csv", header=False
        )
    else:
    ...
    if parser_args.export_to_torchscript:
        ...
        instanseg = torch.jit.load(
            "../torchscripts/" + parser_args.model_folder + ".pt"
        )
    if parser_args.export_to_bioimageio:
        ...
        export_bioimageio(
            instanseg,
            deepimagej=True,
            test_img_path="../examples/HE_example.tif",
            model_name=parser_args.model_folder,
        )

    pred_masks, gt_masks, time_dict = instanseg_inference(
        val_images,
        val_labels,
        model,
    ...
    pd.DataFrame(time_dict["combined"]).to_csv(
        output_path / "timing_dict.csv", header=True
    )
    ...
    if parser_args.cells_and_nuclei:
        ...
        compute_and_export_metrics(
            gt_nuclei_masks, pred_nuclei_masks, output_path, target="Nuclei"
        )
        compute_and_export_metrics(
            gt_cell_masks, pred_cell_masks, output_path, target="Cells"
        )
    else:
        pred_masks = [
