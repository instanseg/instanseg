...
parser.add_argument(
    ...
    help="Whether to add a channel invariant model to the pipeline",
)
parser.add_argument(
    "-target",
    "--target_segmentation",
...
parser.add_argument(
    ...
    "--loss_function",
    type=str,
    default="instanseg_loss",
    help="Method to use for segmentation, only instanseg_loss is supported",
)
parser.add_argument(
    "-n_sigma",
    "--n_sigma",
    type=int,
    default=2,
    help="Number of sigma channels, must be at least 1",
)
parser.add_argument(
    "-cluster",
    "--on_cluster",
...
parser.add_argument(
    "-layers", "--layers", type=str, default="[32, 64, 128, 256]", help="UNet layers"
)
parser.add_argument(
    "-slice",
    "--data_slice",
...
parser.add_argument(
    "-clip", "--clip", type=float, default=20, help="Gradient clipping value"
)
parser.add_argument(
    "-decay", "--weight_decay", type=float, default=0.000, help="Weight decay"
)
parser.add_argument(
    "-drop",
    "--dropprob",
...
parser.add_argument(
    ...
    type=float,
    default=0.5,
    help="Intensity transformation factor",
)
parser.add_argument(
    "-dim_in",
    "--dim_in",
...
parser.add_argument(
    ...
    default=False,
    type=lambda x: (str(x).lower() == "true"),
    help="Whether to use cosine annealing for the learning rate",
)
parser.add_argument(
    "-o_h",
...
parser.add_argument(
    ...
    default=10,
    type=int,
    help="Number of epochs to train the model with binary_xloss before starting the main training loop (default=10)",
)
parser.add_argument(
    "-window",
    "--window_size",
...
parser.add_argument(
    "-mlp_w", "--mlp_width", default=5, type=int, help="Width of the MLP hidden dim"
)
parser.add_argument(
    "-augmentation_type",
    "--augmentation_type",
...
parser.add_argument(
    ...
    default="0",
    type=str,
    help="Feature engineering function to use",
)
parser.add_argument(
    "-f",
    "--f",
    default=None,
    type=str,
    help="ignore, this is for jypyter notebook compatibility",
)
parser.add_argument(
    "-rng_seed",
    "--rng_seed",
...
def main(
    ...
    test_loader,
    num_epochs=1000,
    epoch_name="output_epoch",
):
    from instanseg.utils.AI_utils import (
        optimize_hyperparameters,
        train_epoch,
        test_epoch,
    )
    ...
    for epoch in range(num_epochs):
        ...
        if (
            epoch <= 5 and not args.model_folder
        ):  # Training is just starting AND we are not loading a model
        ...
        test_loss, f1_score, test_time = test_epoch(
            ...
            save_str=str(
                args.output_path / str(f"epoch_outputs/{epoch_name}_" + str(epoch))
            ),
        )
        ...
        if epoch % 10 == 0 and args.optimize_hyperparameters:
            best_params = optimize_hyperparameters(
                ...
                verbose=not args.on_cluster,
                show_progressbar=not args.on_cluster,
            )
...
def instanseg_training(segmentation_dataset: Dict = None, **kwargs):
    ...
    os.environ["INSTANSEG_DATASET_PATH"] = os.environ.get(
        "INSTANSEG_DATASET_PATH", str(args.data_path)
    )
    os.environ["INSTANSEG_OUTPUT_PATH"] = os.environ.get(
        "INSTANSEG_OUTPUT_PATH", str(args.output_path)
    )
    ...
    if args.loss_function == "instanseg_loss":
        ...
        method = InstanSeg(
            binary_loss_fn_str=args.binary_loss_fn,
        ...
        )  # binary_xloss, lovasz_hinge dice_loss general_dice_loss
    ...
    if args.model_folder:
        ...
        model, model_dict = load_model_weights(
            model,
        ...
        if not args.channel_invariant:
            ...
            optimizer.param_groups[0][
                "lr"
            ] = args.lr  # you may want to remove this line in the future
    ...
    if args.channel_invariant:
        ...
        if not has_AdaptorNet(model):
            model = AdaptorNetWrapper(
                model, adaptor_net_str=args.adaptor_net_str, norm=args.norm
            )
        optimizer = optim.Adam(
            model.parameters(), lr=args.lr, weight_decay=args.weight_decay
        )
    ...
    train_loader, test_loader = get_loaders(
        train_images, train_labels, val_images, val_labels, train_meta, val_meta, args
    )
    ...
    pd.DataFrame.from_dict(args_dict, orient="index").to_csv(
        args.output_path / "experiment_log.csv", header=False
    )
    ...
    if args.hotstart_training > 0:
        ...
        print(
            "Hotstart for "
            + str(hot_epochs)
            + " epochs with binary_xloss and dice_loss"
        )
        ...
        print(
            "Starting main training loop with",
            args.seed_loss_fn,
            "and",
            args.binary_loss_fn,
        )
    ...
    df = pd.DataFrame(
        {"train_loss": train_losses, "test_loss": test_losses, "f1_score": f1_list}
    )
    df.to_csv(args.output_path / "experiment_metrics.csv", index=False, header=True)

    fig = plot_average(
        train_losses, test_losses, window_size=len(train_losses) // 10 + 1
    )
