...
parser.add_argument(
    ...
    help="Exclude files with this string in their name",
)
parser.add_argument(
    "-pixel_size",
    "--pixel_size",
    type=float,
    default=None,
    help="Pixel size of the input image in microns",
)
parser.add_argument(
    "-recursive",
    "--recursive",
    ...
    help="Look for images recursively at the image path",
)
parser.add_argument(
    "-ignore_segmented",
    "--ignore_segmented",
    default=False,
    type=lambda x: (str(x).lower() == "true"),
    help="Whether to ignore previously segmented images in the image path",
)
...
parser.add_argument(
    ...
    type=int,
    default=512,
    help="tile size in pixels given to the model, only used for large images.",
)
parser.add_argument(
    "-batch_size",
    "--batch_size",
    type=int,
    default=3,
    help="batch size, only useful for large images",
)
parser.add_argument(
    "-save_geojson",
    "--save_geojson",
    type=lambda x: (str(x).lower() == "true"),
    default=False,
    help="Output geojson files of the segmentation",
)
...
def file_matches_requirement(root, file, exclude_str):
    ...
    for e_str in exclude_str:
        ...
        if parser.ignore_segmented:
            if os.path.isfile(
                os.path.join(root, str(Path(file).stem) + prediction_tag + ".tiff")
            ):
                return False
    return True
...
if __name__ == "__main__":
    ...
    if not parser.recursive:
        ...
        files = [
            os.path.join(parser.image_path, file)
            for file in files
            if file_matches_requirement(parser.image_path, file, parser.exclude_str)
        ]
    else:
