...
def get_marker_location(meta):
    ...
    subcellular_location = [
        (
            "N/A"
            if channel.upper() not in markers_info.keys()
            else markers_info[channel.upper()]["Subcellular Location"]
        )
        for channel in stains
    ]
...
class Augmentations(object):
    ...
    def to_tensor(
        self, image, labels=None, normalize=False, amount=None, metadata=None
    ):
        ...
        if labels is not None:
            if isinstance(labels, np.ndarray):
                ...
                for n, lab in enumerate(labels):
                    ...
                    if not (
                        lab == -1
                    ).any():  # convention is to skip labels with a value of -1
    ...
    def normalize(
        ...
        out = percentile_normalize(
            image, subsampling_factor=subsampling_factor, percentile=percentile
        )

        return out, labels

    def extract_hematoxylin_stain(
        self, image: torch.Tensor, labels=None, amount=0, metadata=None
    ):
        ...
        try:
            ...
            normalizer.HERef += (
                (torch.rand_like(normalizer.HERef) - 0.5) * normalizer.HERef * amount
            )
            normalizer.maxCRef += (
                (torch.rand_like(normalizer.maxCRef) - 0.5)
                * normalizer.maxCRef
                * amount
            )
        ...
        out = _move_channel_axis(H) / 255.0
    ...
    def normalize_HE_stains(
        ...
        out = _move_channel_axis(norm) / 255.0
    ...
    def adjust_hue(self, image, labels, amount=0, metadata=None):
        ...
        out = torch.nan_to_num(
            out, nan=0
        )  # This occasionally produces nan values, which we replace with 0
    ...
    def perspective(self, image, labels, amount=0, metadata=None):
        ...
        perspective_transformer = RandomPerspective(
            distortion_scale=amount / 2,
            p=1.0,
            interpolation=torchvision.transforms.InterpolationMode.NEAREST,
            fill=float(
                image.max()
                if metadata["image_modality"] == "Brightfield"
                else image.min()
            ),
        )
    ...
    # @measure_time
    def pseudo_brightfield(
        self, image, labels=None, amount=0, c_nuclei=0, metadata=None, random_seed=None
    ):
        ...
        image, _, c_nuclei = self.extract_nucleus_and_cytoplasm_channels(
            image, labels, c_nuclei, metadata
        )
    ...
    def colourize(
        self, image, labels=None, amount=0, c_nuclei=-1, random_seed=None, metadata=None
    ):
    ...
    def pseudo_imc(self, image, labels, amount=0, metadata=None):
        ...
        if (
            np.min(torch.squeeze(image).shape) == 3
        ):  # Don't want to run the risk of having a brightfield image in the IMC pipeline.
            if self.debug:
        ...
        shape = resize_lab(
            labels, generate_random_label_area(min=30, max=50)
        )  # Rough size for the nuclei in IMC images.
        ...
        amount = 30 - (
            amount * 27
        )  # Assuming we have amount between [0,1], we want to map that to something like [30,3]

        resized_data = torch.poisson(resized_data * amount)

        resized_data = percentile_normalize(
            resized_data, percentile=0.1, subsampling_factor=3
        )
    ...
    def add_noisy_channels(
        self, image, labels=None, max_channels=30, amount=0, metadata=None
    ):
        if self.debug:
            orig = torch.clone(image)
        ...
        new_channels = np.random.choice(
            range(image.shape[0]), new_channels_num, replace=True
        )
        ...
        channel_weights = torch.randint(
            1, amount, size=[new_channels_num, 1, 1], dtype=torch.float32
        )
    ...
    def add_gradient(self, image, labels=None, amount=0, metadata=None):
        ...
        if random.random() > 0.5:
            y = TF.vflip(y)
        out = (
            amount * (np.random.random() * x + np.random.random() * y)
        ) * image.max() + image
        if self.debug:
            print("Gradient")
            show_images([orig, out], titles=["Original", "Transformed"])
    ...
    def channel_subsample(
        ...
        if channel_num > max_channels:
            if c_nuclei is None or c_nuclei > channel_num:
                slice = np.random.choice(
                    np.arange(channel_num), max_channels, replace=False
                )
            else:
                if max_channels == 1:
                    slice = [c_nuclei]
                    ...
                    slice = np.random.choice(
                        np.arange(channel_num)[np.arange(channel_num) != c_nuclei],
                        np.random.randint(min_channels - 1, max_channels - 1),
                        replace=False,
                    )
    ...
    def extract_nucleus_and_cytoplasm_channels(
        self, image, labels=None, c_nuclei=None, metadata=None, amount=None
    ):
        channel_num = image.shape[0]

        if (
            metadata is None
            or "subcellular_location" not in metadata.keys()
            or len(metadata["subcellular_location"]) != channel_num
            or c_nuclei is None
        ):
            return image, labels, c_nuclei

        cytoplasm_channel_ids = [
            "Cytoplasm" in x or "Membrane" in x or "N/A" in x
            for x in metadata["subcellular_location"]
        ]
        ...
        cytoplasm_channels = image[cytoplasm_channel_ids].sum(
            0
        )  # Sum projection of the cytoplasm channels
    ...
    def torch_rescale(
        ...
        shape = (
            (torch.tensor(image.shape[-2:]) * (1 + ((torch.rand(1) - 0.5) * amount)))
            .int()
            .tolist()
        )
        ...
        while (
            ...
            and np.any(np.array(resized_data[0].shape) < self.shape[0])
            and crop
        ):
            pad = int((self.shape[0] - min(resized_data[0].shape)) / 2) + 3
            pad = (
                torch.Tensor([pad, resized_data.shape[1], resized_data.shape[2]])
                .min()
                .int()
                - 1
            )

            resized_data = torch.nn.functional.pad(
                resized_data,
                (pad, pad, pad, pad),
                mode="constant",
            ...
            if labels is not None:
                resized_labels = torch.nn.functional.pad(
                    ...
                    (pad, pad, pad, pad),
                    mode="constant",
                    value=min(resized_labels.min(), 0),
                ).to(labels.dtype)
        ...
        if labels is not None:
            ...
            assert (
                out_image.shape[-2:] == self.shape
                and out_labels.shape[-2:] == self.shape
            )

        if self.debug:
            print("Rescaling")
            show_images(
                [image[0], labels, out_image[0], out_labels],
                titles=["Source", "Target", "Resize source", "Resized target"],
                n_cols=2,
                axes=True,
            )
    ...
    def duplicate_grayscale_channels(self, image, labels, metadata=None):
        ...
        if image.shape[0] == 1 and self.dim_in != 1 and not self.channel_invariant:
            ...
            raise ValueError(
                "Image has {} channels, but model expects {}, check the augmentations pipeline!".format(
                    image.shape[0], self.dim_in
                )
            )
    ...
    def __call__(self, image, labels, meta=None):
        ...
        if meta is not None and "channel_names" in meta.keys():
            ...
            stains = [
                channel_str.split(" ")[0] for channel_str in meta["channel_names"]
            ]
            subcellular_location = [
                (
                    "N/A"
                ...
                if augmentation in [
                    ...
                    if not has_been_normalized:
                        if augmentation != "normalize":
                            _, amount = values
                        image, labels = getattr(self, augmentation)(
                            image, labels, amount=amount, metadata=metadata
                        )
                    ...
                    image, labels = self.pseudo_brightfield(
                        image, labels, c_nuclei=c_nuclei, metadata=metadata
                    )
                    metadata["image_modality"] = "Brightfield"

                elif augmentation == "channel_subsample":
                    _, (min_channels, max_channels) = values
                    image, labels, c_nuclei = self.channel_subsample(
                        image,
                        labels,
                    ...
                    (
                        ...
                        labels,
                        c_nuclei,
                    ) = self.extract_nucleus_and_cytoplasm_channels(
                        image, labels, c_nuclei=c_nuclei, metadata=metadata
                    )

                elif augmentation == "torch_rescale":
                    _, requested_pixel_size, amount = values
                    image, labels = self.torch_rescale(
                        image,
                        labels,
                        current_pixel_size=None,
                    ...
                    _, amount = values
                    image, labels = self.colourize(
                        image, labels, c_nuclei=c_nuclei, metadata=metadata
                    )

                elif augmentation == "add_noisy_channels":
                    _, max_channels = values
                    image, labels = self.add_noisy_channels(
                        image,
                        labels,
                        metadata=metadata,
        ...
        image, labels = self.duplicate_grayscale_channels(
            image, labels, metadata=metadata
        )  # This will only catch single channel images fed to a multi channel network and duplicate the channel if required.

        if image.var() > 1e2:
            image = torch.clip(image, min=-1, max=5)
            warnings.warn(
                "Warning, variance of image is very high, check augmentations"
            )
...
if __name__ == "__main__":
    ...
    augmentation_dict = get_augmentation_dict(
        nuclei_channel=6, dim_in=3, amount=0.5, augmentation_type="heavy"
    )["train"]
    ...
    Augmenter = Augmentations(
        augmentation_dict=augmentation_dict, debug=False, dim_in=None
    )
