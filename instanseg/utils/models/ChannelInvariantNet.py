...
from instanseg.utils.models.InstanSeg_UNet import (
    DecoderBlock,
    EncoderBlock,
    conv_norm_act,
)
...
class ChannelInvariantEncoderBlock(EncoderBlock):
    def __init__(
        ...
        super().__init__(
            in_channels, out_channels, norm=norm, act=act, pool=pool, shallow=False
        )
        self.aggregation = aggregation

        if self.aggregation in ["concat", "no_aggregation"]:
            self.reduce_block = conv_norm_act(
                out_channels * 2, out_channels, sz=3, norm=norm, act=act
            )
    ...
    def forward(self, x, c, b):
        ...
        if self.aggregation == "no_aggregation":
            cat0 = torch.cat(
                (repeat(pool_x0, "b k h w -> b c k h w", c=c) * 0, x0), dim=2
            )
            cat0 = rearrange(cat0, "b c k h w -> (b c) k h w")
            cat0 = self.reduce_block(cat0)
...
class ChannelInvariantDecoderBlock(DecoderBlock):
    def __init__(
        ...
        super().__init__(
            in_channels, skip_channels, out_channels, norm, act, shallow=False
        )
        self.aggregation = aggregation
    ...
    def forward(self, x, skip=None, c=None, b=None):
        ...
        if self.aggregation == "concat":
            cat_u2 = torch.cat(
                (repeat(pool_u2, "b k h w -> b c k h w", c=c), u2), dim=2
            )
        ...
        if self.aggregation == "no_aggregation":
            cat_u2 = torch.cat(
                (repeat(pool_u2, "b k h w -> b c k h w", c=c) * 0, u2), dim=2
            )
            cat_u2 = rearrange(cat_u2, "b c k h w -> (b c) k h w")
            cat_u2 = self.reduce_block(cat_u2)
...
class ChannelInvariantNet(nn.Module):
    ...
    def __init__(
        ...
        self.conv_0 = ChannelInvariantEncoderBlock(
            1, layers[0], act=act, norm=norm, pool=False, aggregation=aggregation
        )

        self.down_1 = ChannelInvariantEncoderBlock(
            layers[0], layers[1], act=act, norm=norm, pool=True, aggregation=aggregation
        )
        self.down_2 = ChannelInvariantEncoderBlock(
            layers[1], layers[2], act=act, norm=norm, pool=True, aggregation=aggregation
        )

        self.upcat_2 = ChannelInvariantDecoderBlock(
            layers[2], layers[1], layers[1], act=act, norm=norm, aggregation=aggregation
        )
        self.upcat_1 = ChannelInvariantDecoderBlock(
            layers[1],
            layers[0],
            layers[0],
            act=act,
        ...
        self.final_conv = conv_norm_act(
            layers[0], out_channels, sz=1, norm=final_norm, act=None
        )
    ...
    if has_AdaptorNet(model):
        ...
        if adaptornet is not None:
            ...
            if adaptor_net_str == "1":
                ...
                model.AdaptorNet = ChannelInvariantNet(
                    aggregation="no_aggregation", **kwargs
                )
        return model
...
class _AdaptorNetWrapper(torch.nn.Module):
    def __init__(self, model, adaptornet=None, adaptor_net_str="1", **kwargs):
        ...
        self.model = initialize_AdaptorNet(
            model, adaptornet=adaptornet, adaptor_net_str=adaptor_net_str, **kwargs
        )
...
class AdaptorNetWrapper(_AdaptorNetWrapper):
    def __init__(self, *args, device="cuda", norm=None, **kwargs):
        ...
        if norm is not None and norm.lower() in [
            "instance_invariant",
            "custom_instance_invariant",
        ]:
            _norm = "INSTANCE"
        else:
            _norm = norm
        ...
        for child_name, child in model.named_children():
            if isinstance(child, torch.nn.InstanceNorm2d):
                ...
                child.register_buffer(
                    "running_mean", torch.zeros(child.num_features, device=device)
                )
                child.register_buffer(
                    "running_var", torch.ones(child.num_features, device=device)
                )
                child.register_buffer(
                    "num_batches_tracked", torch.ones(1, device=device)
                )
            else:
