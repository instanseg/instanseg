...
def load_TNBC_2018(Segmentation_Dataset: dict, verbose: bool = True) -> dict:
    ...
    tnbc_dir = create_raw_datasets_dir(
        "Nucleus_Segmentation", "TNBC_NucleiSegmentation"
    )
    zip_file_path = tnbc_dir / "TNBC_NucleiSegmentation.zip"
    download_url = (
        "https://zenodo.org/record/3552674/files/TNBC_and_Brain_dataset.zip?download=1"
    )
            ...
            for file in sorted(os.listdir(folder)):
                ...
                if ".DS_Store" not in str(file):
                    ...
                    labels, remapping = fastremap.renumber(
                        skimage.morphology.label(lab), in_place=True
                    )
    ...
    Segmentation_Dataset["Validation"] += items[
        int(len(items) * 0.8) : int(len(items) * 0.9)
    ]
    ...
    for i, folder in enumerate(tqdm(folders)):
        ...
        for j, file in enumerate(sorted(os.listdir(img_folder))):
            ...
            if ".DS_Store" not in str(file):
                ...
                masks = io.imread(
                    str(file)
                    .replace("tissue images", "label masks modify")
                    .replace(".png", ".tif")
                )
...
def load_IHC_TMA(Segmentation_Dataset: dict, verbose: bool = True) -> dict:
    ...
    download_url = (
        "https://zenodo.org/record/7647846/files/IHC_TMA_dataset.zip?download=1"
    )
...
def load_MoNuSeg(Segmentation_Dataset: dict, verbose: bool = True) -> dict:
    ...
    download_url = (
        "https://github.com/juglab/EmbedSeg/releases/download/v0.1.0/monuseg-2018.zip"
    )
...
def load_CIL(Segmentation_Dataset: dict, verbose: bool = True) -> dict:
    ...
    for file in tqdm(train_folders):
        if "w2" not in file.name:
            ...
            labels = io.imread(
                str(file)
                .replace("CIL_images", "CIL_labels")
                .replace("w1.TIF", "_GT_01.tif")
            )[:, :, 0]
            from skimage.measure import label
...
def load_pannuke(
    Segmentation_Dataset: dict, verbose: bool = True, no_zip=False
) -> dict:
    from pathlib import Path
    import numpy as np
    import fastremap
    ...
    if not (pannuke_dir / "fold_1.zip").exists():
        for fold in ["1", "2", "3"]:
            ...
            download_url = (
                f"https://warwick.ac.uk/fac/cross_fac/tia/data/pannuke/fold_{fold}.zip"
            )
    ...
    def get_data(dataset):
        ...
        items = []
        masks = np.load(
            Path(pannuke_dir) / "Fold {}/masks/fold{}/masks.npy".format(fold, fold),
            allow_pickle=True,
        ).astype(np.int16)
        images = np.load(
            Path(pannuke_dir) / "Fold {}/images/fold{}/images.npy".format(fold, fold),
            allow_pickle=True,
        ).astype(np.uint8)
        types = np.load(
            Path(pannuke_dir) / "Fold {}/images/fold{}/types.npy".format(fold, fold),
            allow_pickle=True,
        )
        ...
        for i in tqdm(range(len(images))):
            ...
            assert (
                np.unique(masks[i, :, :, :-1]) == np.unique(masks[i, :, :, :-1].max(-1))
            ).all()
            label = masks[i, :, :, :-1].max(-1)
            label, _ = fastremap.renumber(label, in_place=True)
            classes = (np.argmax(masks[i, :, :, :-1], axis=-1) + 1) * (
                label > 0
            ).astype(np.int8)
            ...
            tifffile.imwrite(
                out_path / ("nucleus_masks_" + str(i) + ".tif"),
                fastremap.refit(fastremap.renumber(label)[0]),
            )
            tifffile.imwrite(out_path / ("class_masks_" + str(i) + ".tif"), classes)

            relative_path_img = os.path.relpath(
                str(out_path / ("image_" + str(i) + ".tif")),
                os.environ["INSTANSEG_DATASET_PATH"],
            )
            relative_path_nucleus = os.path.relpath(
                str(out_path / ("nucleus_masks_" + str(i) + ".tif")),
                os.environ["INSTANSEG_DATASET_PATH"],
            )
            relative_path_class = os.path.relpath(
                str(out_path / ("class_masks_" + str(i) + ".tif")),
                os.environ["INSTANSEG_DATASET_PATH"],
            )
...
def load_tissuenet(
    ...
    def get_data(dataset):
        ...
        for i in tqdm(range(len(imgs))):
            ...
            tifffile.imwrite(
                out_path / f"cell_masks_{i}.tif",
                fastremap.refit(fastremap.renumber(label[:, :, 0])[0]),
            )
            tifffile.imwrite(
                out_path / f"nucleus_masks_{i}.tif",
                fastremap.refit(fastremap.renumber(label[:, :, 1])[0]),
            )

            relative_path_img = os.path.relpath(
                str(out_path / f"image_{i}.tif"), os.environ["INSTANSEG_DATASET_PATH"]
            )
            relative_path_cell = os.path.relpath(
                str(out_path / f"cell_masks_{i}.tif"),
                os.environ["INSTANSEG_DATASET_PATH"],
            )
            relative_path_nucleus = os.path.relpath(
                str(out_path / f"nucleus_masks_{i}.tif"),
                os.environ["INSTANSEG_DATASET_PATH"],
            )
...
def load_CPDMI_Vectra(Segmentation_Dataset: dict):
    ...
    for subfolder in subfolders:
        subsubfolders = sorted(
            [path for path in Path(subfolder).iterdir() if path.is_dir()]
        )

        for subsubfolder in subsubfolders:

            files = sorted(
                [path for path in Path(subsubfolder).iterdir() if path.is_file()]
            )
            cell_mask_path = [
                path for path in files if "Crop_Cell_Mask_Png" in str(path)
            ]
            nuclei_mask_path = [
                path for path in files if "Crop_Dapi_Mask_Png" in str(path)
            ]
            img_path = [path for path in files if "Crop_Cell_Tif" in str(path)] + [
            ...
            if len(nuclei_mask_path) > 0:
                ...
                connected_nuclei = (
                    K.contrib.connected_components(
                        torch.tensor(nuclei_mask[None]).bool().float(),
                        num_iterations=150,
                    )
                    .int()
                    .numpy()
            ...
            connected = (
                K.contrib.connected_components(
                    torch.tensor(mask[None]).bool().float(), num_iterations=150
                )
                .int()
                .numpy()
            )
            ...
            with TiffFile(img_path[0]) as tif:
                ...
                item["resolution"] = (
                    float(magnification[0].split("=")[1].strip()) / 1000
                )
            ...
            assert len(out_channels) == image.shape[0], print(
                len(out_channels), image.shape[0]
            )

            item["channel_names"] = out_channels
            item["nuclei_channels"] = [
                i for i, val in enumerate(out_channels) if "dapi" in val.lower()
            ]
            ...
            if (
                annotation not in df["Annotation:"].unique()
            ...
            item["tissue_type"] = df[df["Annotation:"] == annotation]["Organ:"].values[
                0
            ]
            item["tumor_type"] = df[df["Annotation:"] == annotation][
                "Tissue Type:"
            ].values[0]
...
def load_CPDMI_CODEX(Segmentation_Dataset: dict):
    ...
    for subfolder in subfolders:
        ...
        if len(nucleus_mask_path) > 0:
            ...
            connected_nuclei = (
                K.contrib.connected_components(
                    torch.tensor(nuclei_mask[None]).bool().float(), num_iterations=150
                )
                .int()
                .numpy()
            )
            connected_nuclei = (
                fastremap.renumber(connected_nuclei)[0].squeeze().astype(np.int16)
            )

        im = image.transpose(0, 3, 1, 2)
        image = im.reshape(
            -1, im.shape[-2], im.shape[-1]
        )  # This was checked by eye and looks correct. The channel ordering was not recorded properly in the metadata.

        if image.shape[-2:] != mask.shape[-2:]:
            print(
                "Dimension inconsistency", image.shape, mask.shape, subfolder, subfolder
            )
            continue
