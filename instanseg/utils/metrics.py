...
def _robust_average_precision(labels, predicted, threshold):
    ...
    if labels[0].shape[0] != 2:  # cells or nuclei
        labels = [
            labels[i].detach().cpu().numpy().astype(np.int32)
            for i, l in enumerate(labels)
            if labels[i].min() >= 0 and labels[i].max() > 0
        ]
        predicted = [
            predicted[i].detach().cpu().numpy().astype(np.int32)
            for i, l in enumerate(labels)
            if labels[i].min() >= 0 and labels[i].max() > 0
        ]
        ...
        for i, _ in enumerate(["nuclei", "cells"]):
            labels_tmp = [
                fastremap.renumber(labels[j][i].detach().cpu().numpy())[0].astype(
                    np.int32
                )
                for j, l in enumerate(labels)
                if labels[j][i].min() >= 0 and labels[j][i].max() > 0
            ]
            predicted_tmp = [
                fastremap.renumber(predicted[j][i].detach().cpu().numpy())[0].astype(
                    np.int32
                )
                for j, l in enumerate(labels)
                if labels[j][i].min() >= 0 and labels[j][i].max() > 0
            ...
            stats = matching.matching_dataset(
                [l for l in labels_tmp],
                [p for p in predicted_tmp],
                thresh=threshold,
                show_progress=False,
            )
            f1i = [stat.f1 for stat in stats]
