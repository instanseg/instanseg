...
def torch_peak_local_max_LEGACY(
    image: torch.Tensor,
    neighbourhood_size: int,
    minimum_value: float,
    return_map: bool = False,
    ...
    retained_maxima = find_all_local_maxima(
        distance_of_max_poses, neighbourhood_size, minimum_value=minimum_value
    )
...
class InstanSeg(nn.Module):
    def __init__(
        ...
        (
            ...
            self.feature_engineering_width,
    ...
    def initialize_pixel_classifier(self, model, MLP_width=10, MLP_input_dim=None):
        ...
        if has_pixel_classifier_model(model):
            try:
                ...
                )  # This happens when there is an adaptornet
            return model
        else:
            if MLP_input_dim is None:
                MLP_input_dim = (
                    self.feature_engineering_width + self.n_sigma - 2 + self.dim_coords
                )
            model.pixel_classifier = ProbabilityNet(MLP_input_dim, width=MLP_width)
            if self.feature_engineering_function != "10":
                model.pixel_classifier = ProbabilityNet(MLP_input_dim, width=MLP_width)
            else:
                model.pixel_classifier = ConvProbabilityNet(
                    MLP_input_dim, width=MLP_width
                )
    ...
    def postprocessing(
        ...
        for i in range(iterations):
            ...
            if precomputed_crops is None:
                ...
                if not self.to_centre:
                    fields = (
                        torch.sigmoid(prediction_i[0 : self.dim_coords]) - 0.5
                    ) * 8
                else:
    ...
    def TTA_postprocessing(
        ...
        if self.cells_and_nuclei:
            iterations = 2
            assert self.dim_out % 2 == 0, print(
                "The model should an even number of output channels for cells and nuclei."
            )
            dim_out = int(self.dim_out / 2)
        else:
            iterations = 1
        ...
        for i in range(iterations):
            ...
            for t in transforms:
                with torch.cuda.amp.autocast():
                    ...
                    augmented_image, pad = _instanseg_padding(
                        augmented_image, extra_pad=0, min_dim=32
                    )
                    prediction = model(augmented_image)[
                        :, i * dim_out : (i + 1) * dim_out
                    ]
            ...
            if reduction == "local_max":
                ...
                local_maxima_map = torch_peak_local_max(
                    torch.stack(local_maxima_maps).max(0)[0].squeeze(),
                    int(peak_distance),
                    seed_threshold,
                    return_map=True,
                )
                all_masks = (
                    torch.mean(torch.stack(all_masks_list), dim=0).float().to(device)
                )

            elif reduction in ["mean", "median"]:
                if reduction == "mean":
                    all_masks = (
                        torch.mean(torch.stack(all_masks_list), dim=0)
                        .float()
                        .to(device)
                    )
                elif reduction == "median":
                    all_masks = torch.median(torch.stack(all_masks_list), dim=0)[0].to(
                        device
                    )
            ...
            if len(centroids) == 0:
                out_labels.append(
                    torch.zeros((1, *all_masks.shape[-2:]), dtype=int, device=device)
                )
                continue

            local_maxima_map[..., centroids[:, 0], centroids[:, 1]] = torch.arange(
                1, centroids.shape[0] + 1, device=local_maxima_map.device
            ).float()
            ...
            for t, prediction in zip(transforms, all_predictions):
                ...
                out = self.postprocessing(
                    ...
                    cleanup_fragments,
                    max_seeds,
                    return_intermediate_objects=True,
            ...
            all_crops = torch.median(torch.stack(all_crops).float(), dim=0)[0].to(
                device
            )
            ...
            labels = self.postprocessing(
                ...
                cleanup_fragments,
                max_seeds,
                precomputed_crops=(all_crops, coords, mask_map),
            )
...
class InstanSeg_Torchscript(nn.Module):
    def __init__(
        ...
        self.default_target_segmentation = self.params.get(
        ...
        self.default_resolve_cell_and_nucleus = self.params.get(
    ...
    def forward(
        ...
        mask_threshold = (
            ...
            else self.default_mask_threshold
        )
        peak_distance = (
            int(peak_distance)
            if peak_distance is not None
            else self.default_peak_distance
        )
        seed_threshold = (
        ...
        overlap_threshold = (
            ...
            else self.default_overlap_threshold
        )
        mean_threshold = (
            float(mean_threshold)
            if mean_threshold is not None
            else self.default_mean_threshold
        )
        window_size = (
            int(window_size) if window_size is not None else self.default_window_size
        )
        cleanup_fragments = (
            bool(cleanup_fragments)
            if cleanup_fragments is not None
            else self.default_cleanup_fragments
        )
        resolve_cell_and_nucleus = (
            bool(resolve_cell_and_nucleus)
            if resolve_cell_and_nucleus is not None
            else self.default_resolve_cell_and_nucleus
        )
        ...
        overlap_threshold = args.get(
            "overlap_threshold", torch.tensor(overlap_threshold)
        ).item()
        mean_threshold = args.get("mean_threshold", torch.tensor(mean_threshold)).item()
        window_size = int(
            args.get("window_size", torch.tensor(float(window_size))).item()
        )
        cleanup_fragments = args.get(
            "cleanup_fragments", torch.tensor(cleanup_fragments)
        ).item()
        resolve_cell_and_nucleus = args.get(
            "resolve_cell_and_nucleus", torch.tensor(resolve_cell_and_nucleus)
        ).item()
        ...
        with torch.no_grad():
            ...
            for image_index in range(x_full.shape[0]):
                ...
                for i in iterations:
                    ...
                    mask_map = (
                        (x[self.dim_coords + self.n_sigma]) / 15
                    ) + 0.5  # inverse transform applied to edt during training.
