...
def get_intersection_over_union(
    label: torch.Tensor, return_lab: bool = True
) -> torch.Tensor:
...
def get_intersection_over_cell_area(
    label: torch.Tensor, return_lab=False
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
...
def get_nonnucleated_cell_ids(
    lab: torch.Tensor,
    iou: torch.Tensor = None,
    threshold: float = 0.5,
    return_lab: bool = True,
) -> torch.Tensor:
    ...
    if return_lab:
        return (
            lab_ids[nonnucleated],
            lab * torch.isin(lab, lab_ids[nonnucleated]),
            lab * torch.isin(lab, lab_ids[~nonnucleated]),
        )
...
def get_nucleated_cell_ids(
    ...
    if return_lab:
        return (
            lab_ids[nucleated],
            lab * torch.isin(lab, lab_ids[nucleated]),
            lab * torch.isin(lab, lab_ids[~nucleated]),
        )
    return lab_ids[nucleated]


def get_multinucleated_cell_ids(
    lab: torch.Tensor,
    iou: torch.Tensor = None,
    threshold: float = 0.5,
    return_lab: bool = True,
...
def resolve_cell_and_nucleus_boundaries(
    ...
    lab = torch.stack(
        ...
        0
    )  # just relabel the nuclei and cells from 1 to N
    ...
    _, lab = keep_only_largest_nucleus_per_cell(
        lab, return_lab=True
    )  # There will now be as many cells as there are nuclei. But the labels are not yet matched
    ...
    if allow_unnucleated_cells:
        ...
        cell_labels[cell_labels == 0] = (
            ...
            cell_labels == 0
        ].float()  # this step can create small fragments. This is not a bug - but may have to be cleaned up in the future.
...
def get_features_by_location(
    input_tensor: torch.Tensor, lab: torch.Tensor, to_numpy: bool = True
) -> tuple:
...
def get_nc_ratio(lab):
    ...
    nuclei_ids, nuclei_areas = torch.unique(
        nuclei_mask[nuclei_mask > 0].flatten(), return_counts=True
    )
    cell_ids, cell_areas = torch.unique(
        cell_mask[cell_mask > 0].flatten(), return_counts=True
    )
    ...
    if len(nc_ratio > 1):
        assert nc_ratio.max() <= 1, "Something whent terribly wrong.{}".format(
            nc_ratio.max()
        )
...
def violin_plot_feature_location(
    X_nuclei: np.ndarray,
                ...
                pd.DataFrame(
                    {
                        ...
                        "Channel": np.repeat(
                            [channel_str, channel_str], [len(array1), len(array2)]
                        ),
                    }
                ),
            ],
    ...
    sns.violinplot(
        data=df, x="Channel", y="Value", hue=" ", split=True, inner="quart", cut=0
    )
