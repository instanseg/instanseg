...
def build_monai_model(model_str: str, build_model_dictionary: dict):
    ...
    if model_str == "AttentionUNet":
        ...
        model = AttentionUnet(
            spatial_dims=2,
            in_channels=int(build_model_dictionary["dim_in"]),
            out_channels=build_model_dictionary["dim_out"],
            dropout=build_model_dictionary["dropprob"],
            channels=build_model_dictionary["layers"],
        ...
        model = FlexibleUNet(
            in_channels=build_model_dictionary["dim_in"],
            out_channels=build_model_dictionary["dim_out"],
            dropout=build_model_dictionary["dropprob"],
...
def read_model_args_from_csv(path=r"../results/", folder=""):
    ...
    if "model_shape" in build_model_dictionary.keys():
        build_model_dictionary["model_shape"] = eval(
            build_model_dictionary["model_shape"]
        )
    for key in ["dim_in", "n_sigma", "dim_out", "dim_coords"]:
        build_model_dictionary[key] = (
            eval(str(build_model_dictionary[key]))
            if str(build_model_dictionary[key]) != "nan"
            else None
        )
    ...
    if "requested_pixel_size" in build_model_dictionary.keys():
        build_model_dictionary["pixel_size"] = float(
            build_model_dictionary["requested_pixel_size"]
        )
    if "cells_and_nuclei" in build_model_dictionary.keys():
        build_model_dictionary["cells_and_nuclei"] = bool(
            eval(build_model_dictionary["cells_and_nuclei"])
        )
    if "norm" in build_model_dictionary.keys():
        if (
            build_model_dictionary["norm"] == "None"
            or str(build_model_dictionary["norm"]).lower() == "nan"
        ):
            build_model_dictionary["norm"] = None
        else:
    ...
    if "feature_engineering" in build_model_dictionary.keys():
        build_model_dictionary["feature_engineering"] = str(
            build_model_dictionary["feature_engineering"]
        )
    else:
    ...
    if "adaptor_net_str" in build_model_dictionary.keys():
        build_model_dictionary["adaptor_net_str"] = str(
            build_model_dictionary["adaptor_net_str"]
        )
    if "multihead" in build_model_dictionary.keys():
        build_model_dictionary["multihead"] = bool(
            eval(build_model_dictionary["multihead"])
        )
    else:
        build_model_dictionary["multihead"] = False
    if "channel_invariant" in build_model_dictionary.keys():
        build_model_dictionary["channel_invariant"] = bool(
            eval(build_model_dictionary["channel_invariant"])
        )
...
def build_model_from_dict(build_model_dictionary):
    if (
        build_model_dictionary["dim_in"] == 0
        or build_model_dictionary["dim_in"] is None
    ):
    ...
    if build_model_dictionary["model_str"] == "InstanSeg_UNet":
        ...
        if build_model_dictionary["cells_and_nuclei"]:
            if not multihead:
                ...
                out_channels = [
                    [
                        build_model_dictionary["dim_coords"],
                        build_model_dictionary["n_sigma"],
                        1,
                    ]
                    for i in range(2)
                ]
        ...
        model = build_monai_model(
            build_model_dictionary["model_str"], build_model_dictionary
        )
...
def load_model_weights(model, device, folder, path=r"../models/", dict=None):
    ...
    model_dict["model_state_dict"] = remove_module_prefix_from_dict(
        model_dict["model_state_dict"]
    )

    if has_pixel_classifier_state_dict(
        model_dict["model_state_dict"]
    ) and not has_pixel_classifier_model(model):
        from instanseg.utils.loss.instanseg_loss import InstanSeg

        method = InstanSeg(
            n_sigma=int(dict["n_sigma"]),
            feature_engineering_function=dict["feature_engineering"],
            dim_coords=dict["dim_coords"],
            device=device,
        )
        model = method.initialize_pixel_classifier(
            model, MLP_width=int(dict["mlp_width"])
        )
    ...
    if has_adaptor_net_state_dict(
        model_dict["model_state_dict"]
    ) and not has_AdaptorNet(model):
        from instanseg.utils.models.ChannelInvariantNet import (
            AdaptorNetWrapper,
            has_AdaptorNet,
        )
        ...
        model = AdaptorNetWrapper(
            model, norm=dict["norm"], adaptor_net_str=dict["adaptor_net_str"]
        )
