...
def _keep_images(item, args):
    if (
        args.source_dataset != "all"
        and item["parent_dataset"] not in args.source_dataset
    ):  # remove items that are not of the desired dataset
        return False
    elif (
        "duplicate" in item.keys() and item["duplicate"]
...
def _format_labels(item, target_segmentation):
    ...
    if target_segmentation == "N":
        ...
        raise NotImplementedError(
            "Target segmentation not recognized", target_segmentation
        )
...
def export_dataset_dict_as_folder(dataset, destination="benchmarking_data"):
    ...
    for fold_name, fold in dataset.items():
        for i, data_info in tqdm(enumerate(fold), ncols=100, total=len(fold)):
            ...
            tifffile.imwrite(
                os.path.join(storage_folder, f"img{i}.tiff"), _move_channel_axis(img)
            )
    ...
    for df_name, df in dataset_dfs.items():
        ...
        pd.DataFrame(df).to_csv(
            os.path.join(expanded_datasets, df_name, f"{df_name}_metadata.csv")
        )


def _read_images_from_path(
    data_path, dataset, data_slice, dummy, args, sets=["Train", "Validation"]
):
    # from pathlib import Path
        ...
        if Path(img_path).exists():
            ...
            if Path(str(Path(img_path).parents[1]) + ".zip").exists():
                ...
                shutil.unpack_archive(
                    str(Path(img_path).parents[1]) + ".zip", Path(img_path).parents[1]
                )
...
def _read_images_from_pth(
    ...
    if complete_dataset is None:
        if not os.environ.get("INSTANSEG_DATASET_PATH"):
            os.environ["INSTANSEG_DATASET_PATH"] = Path(
    ...
    for set in sets:
        ...
        images_local = [
            ...
            for item in complete_dataset[set]
            if _keep_images(item, args)
        ][:data_slice]

        labels_local = [
            _format_labels(item, target_segmentation=args.target_segmentation)
            for item in complete_dataset[set]
            if _keep_images(item, args)
        ][:data_slice]
        metadata = [
    ...
    for set in sets:
        ...
        assert (
            len(data_dicts[set][0]) > 0
        ), "No images in the dataset meet the requirements. (Hint: Check that the source argument is correct)"
...
def get_loaders(
    ...
    train_data = Segmentation_Dataset(
        train_images_local,
        train_labels_local,
        metadata=train_meta,
        size=(256, 256),
    ...
    test_data = Segmentation_Dataset(
        val_images_local,
        val_labels_local,
        size=(256, 256),
        metadata=val_meta,
    ...
    test_sampler = RandomSampler(test_data, num_samples=int(args.length_of_epoch * 0.2))
    ...
    if not args.weight:
        if args.length_of_epoch is not None:
            ...
            test_sampler = RandomSampler(
                test_data, num_samples=int(args.length_of_epoch * 0.2)
            )  # This is relates to the standard 80/20 split
        else:
        ...
        if args.length_of_epoch is not None:
            test_sampler = WeightedRandomSampler(
                rel_freq, int(args.length_of_epoch * 0.2)
            )
        else:
