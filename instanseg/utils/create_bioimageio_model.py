...
def modify_yaml_for_qupath_config(
    yaml_path, pixel_size: float, dim_in: int = 3, dim_out: int = 2, version: str = None
):
    ...
    shutil.copyfile(
        os.path.join(
            os.path.dirname(__file__), "./rdf_scripts/instanseg_preprocess.ijm"
        ),
        os.path.join(os.path.dirname(yaml_path), "instanseg_preprocess.ijm"),
    )
    shutil.copyfile(
        os.path.join(
            os.path.dirname(__file__), "./rdf_scripts/instanseg_postprocess.ijm"
        ),
        os.path.join(os.path.dirname(yaml_path), "instanseg_postprocess.ijm"),
    )
    ...
    data["config"]["deepimagej"] = {
        ...
        "model_keys": None,
        "prediction": {
            "preprocess": [{"kwargs": "instanseg_preprocess.ijm"}],
            "postprocess": [{"kwargs": "instanseg_postprocess.ijm"}],
        },
...
def export_bioimageio(
    torchsript: torch.jit._script.RecursiveScriptModule,
    model_name: str,
    test_img_path: str,
    ...
    try:
        model, model_dict = load_model(
            model_name, path=os.environ["INSTANSEG_MODEL_PATH"]
        )
    except:
    ...
    if img.physical_pixel_sizes.X is not None:
        ...
        print(
            "Pixel size was found in the metadata, pixel size is set to: ", pixel_size
        )
    else:
        pixel_size = 0.5
        print(
            "Pixel size was not found in the metadata, please set the pixel size of the input image in microns manually"
        )
    ...
    )  # this converts the input data to a tensor and does percentile normalization (no clipping)
    ...
    input_crop, _ = Augmenter.torch_rescale(
        ...
        requested_pixel_size=model_pixel_size_tmp,
        crop=True,
        random_seed=1,
    )
    ...
    if input_crop.shape[1] != dim_in and not model_dict["channel_invariant"]:
        ...
        input_crop = torch.zeros(
            (1, dim_in, input_crop.shape[2], input_crop.shape[3]),
            dtype=torch.float32,
            device=input_crop.device,
        )
    ...
    _ = build_model(
        ...
        tags=[
            ...
            "instanseg",
            "whole-slide-imaging",
        ],  # the tags are used to make models more findable on the website
        cite=[
            {
                "text": "Goldsborough, T. et al. (2024) InstanSeg: an embedding-based instance segmentation algorithm optimized for accurate, efficient and portable cell segmentation. _arXiv_. Available at: https://doi.org/10.48550/arXiv.2408.15954",
                "doi": "https://doi.org/10.48550/arXiv.2408.15954",
            },
            {
                "text": "Goldsborough, T. et al. (2024) A novel channel invariant architecture for the segmentation of cells and nuclei in multiplexed images using InstanSeg. _bioRxiv_, p. 2024.09.04.611150. Available at: https://doi.org/10.1101/2024.09.04.611150.",
    ...
    my_model = bioimageio.core.load_resource_description(
        os.path.join(output_path, output_name + ".zip")
    )
    test_model(my_model)
