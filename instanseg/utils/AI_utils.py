...
from instanseg.utils.metrics import (
    _robust_average_precision,
    _robust_f1_mean_calculator,
)
...
def test_epoch(
    ...
    current_f1_list = []
    with torch.no_grad():
        for image_batch, labels_batch, _ in tqdm(
            test_dataloader, disable=args.on_cluster
        ):
            ...
            if (
                ...
                and labels.type() != "torch.FloatTensor"
            ):
                predicted_labels = torch.stack(
                    [postprocessing_fn(out) for out in output]
                )
                f1i = _robust_average_precision(
                    labels.clone(), predicted_labels.clone(), threshold=iou_threshold
                )
    ...
    if mean_f1 > best_f1 or save_bool:
        ...
        if lab.squeeze().dim() == 2:
            show_images(
                [input1]
                + [label_i for label_i in labels_dst]
                + [lab]
                + [out for out in output[0]],
                save_str=save_str,
                titles=["Source"]
                + ["Label" for _ in labels_dst]
                + ["Prediction"]
                + ["Out" for _ in output[0]],
                labels=[1, 2],
            )
        else:
            show_images(
                [input1]
                + [label_i for label_i in labels_dst]
                + [label_i for label_i in lab]
                + [out for out in output[0]],
                save_str=save_str,
                titles=["Source"]
                + ["Label: Nuclei", "Label: Cells"]
                + ["Prediction: Nuclei", "Prediction: Cells"]
                + ["Out" for _ in output[0]],
                labels=[1, 2, 3, 4],
                n_cols=5,
            )
...
class Segmentation_Dataset:
    def __init__(
        ...
        assert len(self.X) == len(self.metadata), print(
            "The number of images and metadata must be the same"
        )
        self.size = size
...
def plot_loss(_model):
    ...
    names = [
        name for name, param in _model.named_parameters() if param.grad is not None
    ]
...
def optimize_hyperparameters(
    ...
    space = {  # instanseg
        ...
        "mean_threshold": hp.uniform("mean_threshold", 0.0, 0.3),
    }  # the max could be increased, but may cuase the method not to converge for some reason.
    ...
    with torch.no_grad():
        if data_loader is not None:
            for image_batch, labels_batch, _ in data_loader:
                ...
                predictions.extend(
                    [pred, masks] for pred, masks in zip(output, labels_batch)
                )
            ...
            def objective(params={}):
                ...
                mean_f1 = _robust_average_precision(
                    torch.stack(gt_masks), torch.stack(pred_masks), threshold=threshold
                )
            ...
            def objective(params={}):
                ...
                mean_f1 = _robust_average_precision(
                    gt_masks, pred_masks, threshold=threshold
                )
            ...
            raise ValueError(
                "Either data_loader or val_images and val_labels must be provided"
            )
