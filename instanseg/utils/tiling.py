...
def _chops(img_shape: tuple, shape: tuple, overlap: int = 0) -> tuple:
    ...
    assert (
        shape[0] > overlap and shape[1] > overlap
    ), f"The overlap {overlap} must be smaller than the window size {shape}"
    ...
    for i, window_i in enumerate(h_index):
        for j, window_j in enumerate(v_index):
            current_window = image[
                ..., window_i : window_i + stride_h, window_j : window_j + stride_v
            ]
...
def _stitch(
    tiles: list, shape: tuple, chop_list: list, final_shape: tuple, offset: int
):
    ...
    for i, window_i in enumerate(chop_list[0]):
        for j, window_j in enumerate(chop_list[1]):
            ...
            if i == len(chop_list[0]) - 1 and j == len(chop_list[1]) - 1:
                ...
                tile1 = canvas[
                    ...,
                    window_i + offset : window_i + shape[0],
                    window_j + offset : window_j + shape[1],
                ]
                tile2 = _remove_edge_labels(
                    new_tile[:, offset : shape[0], offset : shape[1]],
                    ignore=ignore_list,
                )
            ...
            if i == 0 and j == 0:
                edge_window = True
                tile1 = canvas[
                    ..., window_i : window_i + shape[0], window_j : window_j + shape[1]
                ]
                tile2 = _remove_edge_labels(
                    new_tile[:, : shape[0], : shape[1]], ignore=ignore_list
                )
            elif i == 0:
                edge_window = True
                tile1 = canvas[
                ...
                tile2 = _remove_edge_labels(
                    new_tile[:, : shape[0], offset : shape[1]], ignore=ignore_list
                )
                ...
                tile2 = _remove_edge_labels(
                    new_tile[:, offset : shape[0], : shape[1]], ignore=ignore_list
                )
            ...
            if edge_window:
                ...
                tile1 = canvas[
                    ...,
                    window_i + offset : window_i + shape[0] - offset,
                    window_j + offset : window_j + shape[1] - offset,
                ]
                tile2 = _remove_edge_labels(
                    new_tile[:, offset : shape[0] - offset, offset : shape[1] - offset]
                )
        ...
        for j, window_j in enumerate(chop_list[1]):
            for n in range(n_dim):
                ...
                features = labels_to_features(
                    ...
                    offset=[window_j * scale, window_i * scale],
                    downsample=scale,
                )
    ...
    **kwargs,
    ...
    with torch.no_grad():
        with torch.amp.autocast("cuda"):
            batch_list = [
                torch.stack(tile_list[batch_size * i : batch_size * (i + 1)])
                for i in range(int(np.ceil(len(tile_list) / batch_size)))
            ]
            label_list = torch.cat(
                [
                    predictor(tile.to(sw_device), **kwargs).to(device)
                    for tile in tqdm(
                        batch_list,
