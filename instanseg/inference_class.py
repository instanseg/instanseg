...
def to_ndim(x: torch.Tensor, n: int) -> torch.Tensor:
    ...
    if x.dim() != n:
        raise ValueError(
            f"Input tensor has shape {x.shape}, which is not compatible with the desired dimension {n}."
        )
    return x
...
def _to_tensor_float32(image: Union[np.ndarray, torch.Tensor]) -> torch.Tensor:
    ...
    assert (
        image.dim() <= 3 and image.dim() >= 2
    ), f"Input image shape {image.shape()} is not supported."

    image = torch.atleast_3d(image)
    channel_index = np.argmin(
        image.shape
    )  # Note, this could break for small, highly multiplexed images.
    if channel_index != 0:
...
def _rescale_to_pixel_size(
    image: torch.Tensor, requested_pixel_size: float, model_pixel_size: float
) -> torch.Tensor:
...
class InstanSeg:
    ...
    def __init__(
        ...
        headers = (
            None if github_token is None else {"Authorization": "token " + github_token}
        )
        if isinstance(model_type, nn.Module):
            self.instanseg = model_type
        else:
            self.instanseg = download_model(
                model_type, verbose=self.verbose, headers=headers
            )
        ...
        self.small_image_threshold = (
            3 * 1500 * 1500
        )  # max number of image pixels to be processed on GPU.
        self.medium_image_threshold = (
            10000 * 10000
        )  # max number of image pixels that could be loaded in RAM.
        self.prediction_tag = "_instanseg_prediction"

    def read_image(
        self, image_str: str
    ) -> Union[Tuple[str, float], Tuple[np.ndarray, float]]:
        """
        ...
        if (
            img_pixel_size is None
            or float(img_pixel_size) < 0
            or float(img_pixel_size) > 2
        ):
        ...
        if img_pixel_size is not None:
            ...
            if float(img_pixel_size) <= 0 or float(img_pixel_size) > 2:
                warnings.warn(
                    f"Pixel size {img_pixel_size} microns per pixel is invalid."
                )
                img_pixel_size = None
    ...
    def save_output(
        ...
        if save_overlay:
            ...
            ), "Image array must be provided to save overlay."
    ...
    def eval_small_image(
        ...
        **kwargs,
        ...
        if pixel_size is not None:
            ...
            if (
                original_shape[-2] != image.shape[-2]
                or original_shape[-1] != image.shape[-1]
            ):
        ...
        with torch.amp.autocast("cuda"):
            instanseg_kwargs = {
                ...
                if k
                not in ["batch_size", "tile_size", "normalisation_subsampling_factor"]
            }
            instances = self.instanseg(
                image, target_segmentation=target_segmentation, **instanseg_kwargs
            )
    ...
    def eval_medium_image(
        ...
        **kwargs,
        ...
        if pixel_size is None:
            ...
            warnings.warn(
                "Pixel size not provided, this may lead to innacurate results."
            )
        else:
            image = _rescale_to_pixel_size(image, pixel_size, self.instanseg.pixel_size)
        ...
        if normalise:
            image = percentile_normalize(
                image, subsampling_factor=normalisation_subsampling_factor
            )
    ...
    def eval_whole_slide_image(
        ...
        **kwargs,
        ...
        memory_block_size = (
            int(self.medium_image_threshold**0.5),
            int(self.medium_image_threshold**0.5),
        )
        ...
        dims = (
            int(dims[1] / scale_factor),
            int(dims[0] / scale_factor),
        ...
        for _, ((i, window_i), (j, window_j)) in tqdm(
            ...
            intermediate_shape = (
                int(shape[0] * intermediate_to_final),
                int(shape[1] * intermediate_to_final),
            )

            input_data = slide.read_region(
                (int(window_j * scale_factor), int(window_i * scale_factor)),
                best_level,
                (int(intermediate_shape[0]), int(intermediate_shape[1])),
                as_array=True,
            )

            input_tensor = self._to_tensor(input_data)

            new_tile = self.eval_medium_image(
                input_tensor,
            ...
            for n in range(num_iter):
                ...
                if i == len(chop_list[0]) - 1 and j == len(chop_list[1]) - 1:
                    tile1 = canvas[
                        n,
                        window_i + pad : window_i + shape[0],
                        window_j + pad : window_j + shape[1],
                    ]
                    tile2 = _remove_edge_labels(
                        new_tile[n, pad : shape[0], pad : shape[1]], ignore=ignore_list
                    )
                    ...
                    tile1 = canvas[
                        n,
                        window_i : window_i + shape[0],
                        window_j : window_j + shape[1],
                    ]
                    tile2 = _remove_edge_labels(
                        new_tile[n, : shape[0], : shape[1]], ignore=ignore_list
                    )
                elif i == 0:
                    tile1 = canvas[
                        n,
                    ...
                    tile2 = _remove_edge_labels(
                        new_tile[n, : shape[0], pad : shape[1]], ignore=ignore_list
                    )
                    ...
                    tile2 = _remove_edge_labels(
                        new_tile[n, pad : shape[0], : shape[1]], ignore=ignore_list
                    )

                if (
                    j == 0
                    or i == 0
                    or j == len(chop_list[1]) - 1
                    or i == len(chop_list[0]) - 1
                ):
    ...
    def display(self, image: torch.tensor, instances: torch.Tensor) -> np.ndarray:
        """
    ...
    def _cluster_instances_by_mean_channel_intensity(
        self, image_tensor: torch.Tensor, labeled_output: torch.Tensor
    ):
        ...
        mapping = fastremap.component_map(
            np.arange(1, len(adata.obs["leiden"]) + 1),
            adata.obs["leiden"].astype(np.int64) + 1,
        )
